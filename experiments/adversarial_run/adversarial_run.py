...
from .posterior_stats import posterior_sigma_stats
from .reporting import full_report, compare_reports
from .utils import value_tag, write_json
...
def balanced_subset(x, y, n_per_class) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Also returns the indices, so the subset can be located inside x."""
    idx = np.concatenate([
...
def parse_args():
    ...
    ap.add_argument(
        ...
        help="Split all reported metrics are computed on",
    )
...
def main():
    ...
    x_cert, y_cert, _ = balanced_subset(x_eval, y_eval, cfg.cert.per_class)
