...
from experiments.attack_eval import evaluate, pgd_examples
...
# Sub-stream for the predictive draws on the certification subset. Shared
# between the clean and the attacked measurement so that U(x_adv) - U(x) is
# taken across identical posterior samples.
_CERT_EVAL_OFFSET = 7
...
def _pgd_section(model, posterior, *, x_eval, y_eval, cert_idx, y_cert,
                 seed, cert_eval_key) -> tuple[dict, dict]:
    """
    One attack pass per radius over the whole eval split.

    The certification subset is a slice of x_eval, and PGD is per-example, so
    x_adv[cert_idx] IS the attacked certification subset -- no second attack.
    Aggregate robust accuracy and the per-input subset analysis therefore
    describe the same adversarial examples, which they did not before.

    Every radius on CONFIG.attack.pgd_eps is measured, including radii outside
    the certification grid, so the subset uncertainty can be traced across the
    whole grid rather than only at the certified points.
    """
    pgd_eps, cert_eps = _check_eps_grids()
    pgd, hits = {}, {}

    for eps_key, eps in pgd_eps.items():
        x_adv = pgd_examples(model, x_eval, y_eval, eps=eps, attack_seed=seed)

        report = evaluate(
            model, x_adv, y_eval,
            seed=seed + CONFIG.attack.eval_seed_offset,
        )
        adv_uncertainty = _cert_uncertainty(
            posterior, x_adv[cert_idx], y_cert, key=cert_eval_key,
        )
...
def full_report(model, *, x_eval, y_eval, cert_idx, seed,
                pgd, cert, pgd_candidates) -> dict:
    """
    The certification subset is addressed by index into the eval split, so the
    clean report, the attacked report and the certification probe are all
    guaranteed to be talking about the same inputs in the same order.
    """
    posterior = Posterior(model)
    x_cert, y_cert = x_eval[cert_idx], y_eval[cert_idx]
    ...
    if pgd:
        report["pgd"], report["cert_subset_pgd_hit"] = _pgd_section(
            ...
            cert_idx=cert_idx, y_cert=y_cert,
